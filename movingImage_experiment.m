...
predictableSubsetRatio = 1.0; % Ratio of trials with a predictable task
...
imageFile = 'myImage.png';  % Update this path with the actual image file you are using
theImage = imread(imageFile);

% Get the size of the image
[s1, s2, ~] = size(theImage);
imageRect = [0 0 s2 s1];  % Define the rectangle for the image based on its size

% Scale the image if needed
scaleFactor = 0.04;  % Change this to resize the image if needed
imageRect = CenterRectOnPointd(imageRect * scaleFactor, xCenter, yCenter);
...
for trial = 1:numTrials
    ...
    for frame = 1:numFrames
        ...
        centeredRect = CenterRectOnPointd(imageRect, xPos, yPos);
